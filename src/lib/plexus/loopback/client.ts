...
class LoopbackClientImpl implements LoopbackClient {
  ...
  async permit(input: unknown, toolName: string, toolUseId: string): Promise<String> {
    const stream = this.rpc.call('loopback.permit', { input, tool_name: toolName, tool_use_id: toolUseId });
    return collectOne<String>(stream);
  }

  async respond(approvalId: string, approve: boolean, message?: string | null): Promise<RespondResult> {
    const stream = this.rpc.call('loopback.respond', { approval_id: approvalId, approve, message });
