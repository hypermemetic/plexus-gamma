...
export class PlexusRpcClient implements RpcClient {
  ...
  private handleMessage(data: string): void {
    ...
    // Subscription notifications: detected by params.subscription presence (method
    // name varies across substrate versions — was 'subscription', now the call method).
    if (!('id' in obj) && typeof (obj.params as Record<string,unknown>)?.subscription === 'number') {
  ...
  private handleResponse(resp: JsonRpcResponse): void {
    ...
    if ('error' in resp) { const { code, message, data } = resp.error; pending.reject(new Error(`RPC error ${code}: ${message}${data !== undefined ? ` — ${JSON.stringify(data)}` : ''}`)); }
